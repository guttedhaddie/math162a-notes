...
The \emph{tangent space at $p$} is the set $T_p\R^3$ of all tangent vectors based at $p$. Euclidean space $\R^3$ has a \emph{different tangent space at each point}!%\medbreak
%The collection of all tangent spaces at all points is denoted $T\R^3$ and is called the \emph{tangent bundle.}
\tcblower
\flushright\includegraphics{euclid-tanvec}
\end{defn}
...
Surfaces are the focus of the second half of the course. %In anticipation, hopefully  Its components are the scalar functions
...
% 
% \begin{defn}{}{}
% A \emph{vector field} on $U\subseteq\R^3$ is a function $v:U\to T\R^3$ that assigns a tangent vector at each point of $U$:
% \[\forall p\in U,\ \ v(p)\in T_p\R^3\]
% \end{defn}
% 
% 
% 
% \begin{example}{}{}
% In suitable units, the famous \emph{inverse square field} from physics is the function
% \[v=\frac{-1}{(x^2+y^2+z^2)^{3/2}}\threevec xyz\qquad \text{whenever $(x,y,z)\neq (0,0,0)$}\]
% For instance, at $p=(1,2,2)$, the vector field returns the tangent vector $v(1,2,2)=\frac{-1}{27}\sthreevec 122\in T_p\R^3$.\par
% Each tangent vector $v(p)$ points towards the origin and has length
% \[\Nm{v(p)}=\frac 1{x^2+y^2+z^2}=\frac 1{\Nm p^2}\]
% the reciprocal (`inverse') of the square of the distance of $p$ from the origin.
% \end{example}
% 
% Continuing the observation ($\ast$), if $v,w$ are vector fields, then at each point $p\in\R^3$, the vectors $v(p),w(p)$ \emph{lie in the same vector space} $T_p\R^3$ and can therefor be added. We can therefore add vector fields and multiply them by scalars in a natural way. 
% 
% 
% \begin{example}{}{}
% If $v=\sthreevec{2x}{\cos z}{1-xy}$ and $w=\sthreevec{-xy^2}{2}{2xy+3}$, then $2v+w=\sthreevec{4x-xy^2}{2\cos z+2}{5}$.
% \end{example}
% 
...
%\subsection{Parametrized Curves in Euclidean space}
...
\begin{minipage}[t]{0.72\linewidth}\vspace{0pt}
  ...
  To help visualize this, imagine sitting on top of the $z$-axis and looking down; you'd see its horizontal projection $t\mapsto\bigl(\cos t,\sin t\bigr)$ (a counter-clockwise circle). Since $z(t)=t$, the curve moves upwards at constant speed. One can similarly project onto the $xz$- and $yz$-planes.\bigbreak
	...
	\makebox[0cm][l]{\includegraphics[scale=1]{curves-helix4}}
...
By the chain rule, $\diff[\vy]{s}=\alpha'(s)\diff[\vx]{t}$, which is non-zero by assumption.%\smallbreak
%Moreover, if $\alpha$ is onto, then $\range\alpha=I\implies \range\vy=\range\vx$.
\end{proof}
...
% 		\flushright\includegraphics{regular-catenary}
% 	\end{minipage}
...
%   \item	We saw two parametrizations of the unit circle in Example \ref{ex:circlebasic}. Show that $\vy(s)=\vx\bigl(\alpha(s)\bigr)$, where $\alpha:\R\to(-\pi,\pi):s\mapsto 2\tan^{-1}s$.\par
%   (\emph{Hint: ignore the point $(-1,0)$ and try the reverse $\vx(t)=\vy\bigl(\alpha^{-1}(t)\bigr)$})
	
% 	\item Consider the spiral $\vx(\theta)=\twovec{\theta\cos\theta}{\theta\sin\theta}$, where $\theta\ge 0$ is the angle a point on the curve makes with the $x$-axis. Show that the arc-length of the spiral over its first circuit of the origin (from $\theta=0$ up to $2\pi$) is $\pi\sqrt{1+4\pi^2}+\frac 12\sinh^{-1}2\pi$.\\
% 	(\emph{This is a nasty integration by substitution and will be very difficult without some knowledge of hyperbolic functions, their derivatives and double angle formulae})
...
% \begin{aside}
% {\bfseries Aside: a non-regularizable curve}\\
%  Example 3, above, cannot be made regular via any reparameterization. Indeed let $\alpha(s)$ be an increasing, differentiable function such that $\alpha(0)=0$, and set $\vy(s)=\vx(\alpha(s))$. Then
% \[\vy'(s)=\alpha'(s)\vx'(\alpha(s))\]
% If $\vy$ is to be regular at $s=0$, we should be able to compute $\vy'(0)$. However this is impossible: $\vx'(\alpha(0))=\vx'(0)$ is undefined, whence we would have to attempt to define
% \[\vy'(0)=\lim\limits_{s\to 0}\alpha'(s)\vx'(\alpha(s)) =\alpha'(0)\lim\limits_{s\to 0}\vx'(\alpha(s))\]
% Since $\lim\limits_{s\to 0}\vx'(\alpha(s))$ is also undefined,\footnote{$\lim_{t\to 0^+}\vx'(t)=\twovec 1{-1}\neq\twovec 11=\lim_{t\to 0^-}\vx'(t)$.} we could only have $\alpha'(0)=0$, resulting in $\vy'(0)=\V0$, whence the curve remains non-regular at $t=0$.\\
% The difference between Examples 2 and 3 is simple: you can't regularize corners, but you can regularize smooth curves that have been parametrized so that the curve sometimes comes to a halt.
% \end{aside}
...
\boldsubsubsection{Orthogonal Matrices}
...
\begin{enumerate}\setcounter{enumi}{1}
  ...
  \end{enumerate}
%   \item An orthogonal matrix $A$ has determinant $\det A=\pm 1$. If $\det A=1$ we call the matrix a \emph{rotation,} and if $\det A=-1$ we call it a \emph{reflection.} The set of rotations is denoted\footnote{This is the set of \emph{special orthogonal} matrices.} $\rSO_3(\R)$.
%   \item A moving frame is a function $\vE:I\to\rSO_3(\R)$.
\end{enumerate}
\end{lemm}
...
\footnotetext{Recall that the matrix of a linear map is found by evaluating the map on the standard basis: thus the 1\st\ columns of $A_\theta$ is the column vector $A_\theta\vi=\stwovec{\cos\theta}{\sin\theta}$. The pictures should help you verify the remaining columns; for $B_\theta$ you might find it helpful to consider how the required reflections of the standard basis vectors $\vi,\vj$ may be computed using \emph{rotations.}}
...
\footnotetext{Literally \emph{equal length}; it can be seen that every function $S:\E^3\to\E^3$ which preserves distances between all pairs of points has this form.}
...
To be a little more precise; at each point on the curve, the tangent space $T_{\vx(t)}\E^3$ has a standard basis of tangent vectors $\{\vi_{\vx(t)},\vj_{\vx(t)},\vk_{\vx(t)}\}$, and we can write
...
%$ is an orthogonal matrix-valued function with determinant 1.
\tcblower
...
\begin{proof}
Since $\ve_i\cdot\ve_j$ is constant (equals 0 or 1), the product rule says that%(the product rule works for the dot product!)
...
% 		\ve_1'(t)=-\cos t\ve_2(t)-\ve_3(t)\qquad \ve'_2(t)=\cos t\ve_1(t)+\sin t\ve_3(t)\\
% 		\ve_3'(t)=\ve_1(t)-\sin t\ve_2(t)
	\end{gather*}
...
\begin{enumerate}\setcounter{enumi}{1}
  ...
  \item\label{exs:lieorth}\begin{enumerate}
    \item Let $E(t)$ be a matrix-valued function. Show that $\diff t(E(t))^{-1}=-E^{-1}E'E^{-1}$.
		\item Suppose $E:I\to \rO_3(\R)$ is differentiable and define $W(t):=E^{-1}(t)E'(t)$. Show that $W(t)$ is skew-symmetric ($W^T=-W$).%\par
  %(\emph{By virtue of this relationship, the set of skew-symmetric matrices are sometimes denoted $\fso_3(\R)$: this is an example of the relationship between a \emph{Lie group} and its \emph{Lie algebra,} a foundation on which much of advanced differential geometry rests.})
  \end{enumerate}
...
% 	\item Prove parts 3, 4 and 6 of Theorem 1.11 from the notes (basic properties of vector product).
% 
% 	\item Let $\vx(t)$ and $\vy(t)$ be differentiable curves in $\E^3$. Suppose that the derivatives of $\vx,\vy$ satisfy the following:
% 	\[\vx'(t)=a\vx(t)+b\vy(t),\qquad\vy'(t)=c\vx(t)-a\vy(t),\]
% 	where $a,b,c$ are constants. Show that,
% 	\begin{enumerate}
%   	\item $\vx(t),\vy(t)$ lie in a fixed plane.
%   	\item The area of the parallelogram spanned by $\vx(t),\vy(t)$ is constant.
% 	\end{enumerate}
	
%  	\item Suppose that $\vv$ is a fixed vector in $\E^3$ and let $\va,\vb$ be an orthogonal basis of the plane orthogonal to $\vv$. Describe how to find all the unit vectors (length 1) which are perpendicular to $\vv$ in terms of $\va,\vb$. Use this to find all unit vectors which are perpendicular to the vector $\threevec 110$.
...
\footnotetext{Recall that $\Nm{\va\times\vb}=\Nm\va\Nm\vb\sin\theta$ where $\theta$ is the angle between the vectors.}
...
\end{quote}
The tangent line at any point lies in the osculating plane. %The torsion describes how the osculating plane changes as we move along the curve. 
...
\]\\ %=e^t\threevec{\sqrt 2\cos(t+\frac\pi 4)}{\sqrt 2\sin(t+\frac\pi 4)}1\\
...
As a precursor to this, we first consider how to interpret curvature and torsion, and how they change (or don't!) under rigid motions of a curve.
...
Our classification of spacecurves depends on the `usual' existence and uniqueness result for ODEs. %$Indeed the relationship between curves/surfaces and ODEs is a cornerstone of differential geometry.
...
% Compare the Fundamental Theorem with the distance-speed problem from elementary calculus, where location at all times may be computed from simple data: the initial position $x(t_0)$ and the speed \emph{function} $v(t)$,
% \[x(t)=x(t_0)+\int_{t_0}^tv(u)\,\du\]
% Compare the Fundamental Theorem with the distance-speed problem from elementary calculus: if you know the \emph{velocity} $\vx'(t)$ of a particular, where location at all times may be computed from simple data: the initial position $x(t_0)$ and the speed \emph{function} $v(t)$,
% \[x(t)=x(t_0)+\int_{t_0}^tv(u)\,\du\]
% Our result provides an analogue for spacecurves: given an initial position and orientation (direction and direction of curvature), and functions for speed and how much a curve bends and twists, you can determine the entire motion. For an arbitrary speed curve, the required data is the sextuple\footnote{You don't need explicitly to specify $\vB(t_0)=\vT(t_0)\times\vN(t_0)$!}
% \[\bigl(\vx(t_0),\vT(t_0),\vN(t_0),v(t),\kappa(t),\tau(t)\bigr)\]
...
%   Suppose $\vy(s)=A\vx(s)+\vb$ is the result of such a transform. Then
% \[\vy'(s)=A\vx'(s)\implies \Nm{\vy'(s)}=\Nm{A\vx'(s)}=\Nm{\vx'(s)}=1\]
% since $A$ is orthogonal and thus \emph{length-preserving.} The tangent vector for the transformed curve $\vy(s)$ is therefore\footnote{Remember that $\vT$ and $\hat\vT$ are elements of different tangent spaces: what we mean is that their \emph{direction} parts are equal.}
% \[\hat\vT(s)=A\vT(s)\]
% Since $A$ and $J$ are both rotations, they commute, whence the normal vector of the transformed curve is
% \[\hat\vN(s)=J\hat\vT(s)=JA\vT(s)=AJ\vT(s) =A\vN(s)\]
% Since $A$ is constant, the curvature of the transformed curve is therefore
% \[\hat\kappa=\hat\vT'\cdot \hat\vN=(A\vT')\cdot(A\vN)=\vT'\cdot\cN=\kappa\]
...
% 	. By Exercise \hyperref[exs:frenet2deasy]{\ref*{sec:orth}.\ref*{exs:frenet2deasy}} the Frenet frame is
% 	\[\vT(t):=\twovec{\cos\theta(t)}{\sin\theta(t)},\qquad \vN(t):=J\vT(t)=\twovec{-\sin\theta(t)}{\cos\theta(t)}\]
% 	\begin{enumerate}
% 	  \item Verify that $\{\vT,\vN\}$ is a moving frame for which $w_{12}(t)=-\kappa(t)$.
% 	  \item Integrate to find the unique 
% 	\end{enumerate}
	

% 
% The structure equations for this frame are easy to compute: note how the curvature appears naturally
% \[\vT'=\theta'\twovec{-\sin\theta}{\cos\theta}=\kappa\vN\implies (\vT'\ \ \vN')=(\vT\ \ \vN)\begin{pmatrix}
% 0&-\kappa\\\kappa&0
% \end{pmatrix}\]
% One advantage of the structure equations is that we're able to see what happens to the curve under a rigid motion. The \emph{really} nice thing about this is that we can \emph{reverse} the process.\smallbreak
% 
% Suppose $\kappa:[a,b]\to\R$ is continuous. The Fundamental Theorem of Calculus says that
% \[\theta(s):=\int_a^s\kappa(u)\,\du\]
% is \emph{differentiable} with derivative $\theta'(s)=\kappa(s)$. Since $\theta$ is continuous (and thus so are $\cos\theta$, $\sin\theta$), a second application of the Fundamental Theorem defines a new \emph{vector-valued} function:
% \[\vx(s):=\int_a^s\twovec{\cos\theta(u)}{\sin\theta(u)}\du\]
...
The above suggests that the centers of curvature form an interesting curve.
...
\]
%The picture below is linked to an animation of the construction.
\begin{enumerate}\setcounter{enumi}{1}
...
%These concepts are quite close to being inverses.
...
\begin{enumerate}\setcounter{enumi}{1}
  ...
  (\emph{Hint: this is a trick question!})
	...
	\begin{enumerate}
	  ...
	  %\item Can you modify your argument to cope with the situation when $\kappa$ or $t$ is negative?
	\end{enumerate}
	...
	\begin{enumerate}
	  ...
	  \item Find the involute of the evolute of the parabola $\vx(t)=(t,t^2)$. %this is $\vx(t)=(t,t^2)+\frac 1{2\sqrt{1+4t^2}}(-2t,1)$
	\end{enumerate}	
...
%   
% 
% \item Another famous involute is that of the \emph{cycloid}: it is just itself but shifted!
\end{enumerate}
\end{exercises}
