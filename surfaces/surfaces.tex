...
\footnotetext{A surface is typically parametrized by several overlapping functions $\vx$. Our definition is \emph{local} since there is only one $\vx$.}

%The \emph{Mobius strip} shows that not every surface is orientable!

In what follows, we will often refer to the function $\vx$ as the surface.\smallbreak

% \begin{minipage}[t]{0.5\linewidth}\vspace{0pt}
% Note that the partial derivatives $\vx_u(p),\vx_v(p)$ really are \emph{tangent to the surface} at $\vx(p)$: if $p=(u_0,v_0)$ then the \emph{curve} $\vy(t):=\vx(t,v_0)$ lies in the surface and passes through $P=\vx(p)$; its tangent vector at $P$ is then
% \begin{align*}
% \vy'(u_0)&=\lim_{h\to 0}\frac{\vy(u_0+h)-\vy(u_0)}h\\
% &=\lim_{h\to 0}\frac{\vx(u_0+h,v_0)-\vx(p)}h\\
% &=\partialsat[\vx]{u}p
% \end{align*}
% \end{minipage}\hfill\begin{minipage}[t]{0.49\linewidth}\vspace{0pt}
% \flushright%\includegraphics{surface-tancurve}
% PICTURE of curve in surface and tangent vector or param?
% \end{minipage}
...
The partial derivatives $\vx_u(p),\vx_v(p)$ really are \emph{tangent to the surface} at $\vx(p)$: if $p=(u_0,v_0)$ then the \emph{curve} $\vy(t):=\vx(t,v_0)$ lies in the surface and passes through $P=\vx(p)$; its tangent vector at $P$ is then
\[
...
To help distinguish between the domain and codomain, we standardize notation.
\begin{description}
\item[\normalfont\emph{Domain} $U\subseteq\R^2$:]\begin{description}\itemsep2pt
...
\end{description} 
\item[\normalfont\emph{Codomain} $\E^3$:]\begin{description}\itemsep2pt
...
\footnotetext{To also use our new notation in $\E^3$ would require a subtle redefinition of $\dvx$: if $\vec w$ is a vector field on $U$, then $\dvx(\vec w)$ is the vector field on $S$ such that $\bigl(\dvx(\vec w)\bigr)[f]=\vec w\bigl[f\circ\vx\bigr]$ for all $f:S\to\R$. In co-ordinates this benefits from  tensor notation:
\[\vx(u_1,u_2)=\Bigl(x_1(u_1,u_2),x_2(u_1,u_2),x_3(u_1,u_2)\Bigr)\implies \dvx=\sum_{i,j}\partials[x_j]{u_i}\partials{x_j}\otimes\du_i\]
This approach is necessary in more general situations, but is overkill for our purposes.}
...
% 	This illustrates the term \emph{local}: indeed the famous \emph{hairy ball theorem} from topology says that it is impossible to find a regular parametrization of the entire sphere by a single function.\smallbreak
...
%   Let $\vx:U\to\E^3$ be a surface and $w_p$ a tangent vector at $p\in U$. Consider a curve $c(t)=\bigl(u(t),v(t)\bigr)$ in $U$ such that $c(0)=p$ and $c'(0)=w_p$. Let $\vy(t)=\vx\bigl(c(t)\bigr)$ be the corresponding curve in the surface. Prove that $\dvx(w_p)=\vy'(0)$ so that $\dvx$ really does map tangent vectors to tangent vectors. 
...
It is worth reiterating the two crucially important properties of $\dvx$:
\begin{itemize}
...
\boldinline{Aside: change of co-ordinates}\quad To really spell this out, suppose $\vy(s,t)=\vx\bigl(F(s,t)\bigr)$ where $F(s,t)=(u,v)$ is a change of co-ordinates on $U$. By the chain rule,
\[\twovec{\vy_s}{\vy_t}
=\begin{pmatrix}
...
% You are probably more used to surfaces being defined implicitly---as the level sets of some function---rather than being parametrized.
...
Our next result, a version of the famous \emph{implicit function theorem,} ties together the notions of \emph{regularity.} In particular, it says that we can always assume the existence of \emph{local} co-ordinates.
...
%\[\vx:U\to\E^3:(u,v)\mapsto\bigl(u,v,g(u,v)\bigr)\]
\end{proof}
...
% 	\begin{enumerate}
% 	  \item \emph{Briefly} explain why the unit normal field is $\vn=\cos\phi\,\vN(t)+\sin\phi\,\vB(t)$.
...
% 	\end{enumerate}
  ...
  \end{minipage}\hfill\begin{minipage}[t]{0.24\linewidth}\vspace{-10pt}
  \flushright\href{http://www.math.uci.edu/~ndonalds/math162a/surfaces-helixtube.html}{\includegraphics{surfaces-helixtube}}
  \end{minipage}
  
  
	\item\label{exs:tangentsurface} Let $c(t)=\bigl(u(t),v(t)\bigr)$ be a curve in $U$ and define $\vy(t)=\vx\bigl(c(t)\bigr)$ to be the corresponding curve in the surface $\vx:U\to\E^3$. Prove that $\dvx\bigl(c'(0)\bigr)=\vy'(0)$.\par
...
%   \item Möbius strip: Let $\vx(u,v)=\threevec{\cos u}{\sin u}0(2+v\cos\frac u2) +\threevec 00{v\sin\frac u2}$, $(u,v)\in (0,2\pi)\times (-1,1)$ Compute $\vx_u,\vx_v$ and $\vn$
  
  
%   
%   \begin{minipage}[t]{0.65\linewidth}\vspace{0pt}
% 	\item The tube of radius $a>0$ about a unit speed curve $\vy(u)$ may be parametrized in terms of the Frenet frame of $\vy$:
% 	\[\vx(\phi,u)=\vy(u)+a\cos\phi\vN(u)+a\sin\phi\vB(u)\]
% 	Supposing that $\kappa(u)<\frac 1a$ at all points on the curve, prove that the tube is regular and compute its unit normal field.
% 	\end{minipage}\begin{minipage}[t]{0.35\linewidth}\vspace{-20pt}
% \flushright\includegraphics{surfaces-helixtube}
% \end{minipage}
...
%\footnotetext{$s=\sqrt{x_1^2+\cdots+x_n^2}$ is the distance of a point from the origin.}
...
%We'll consider the meaning of the fundamental forms more fully later. For the present, here are a few simple facts.

\begin{description}
	\item[\normalfont\emph{Basic meaning}]\phantomsection\label{sec:formsmeaning} $\I(\vec v,\vec w)=\dvx(\vec v)\cdot\dvx(\vec w)$ pulls back the dot product from $T_PS$ to $T_p\R^2$. The length of and angle between tangent vectors to $S$ may now be computed in $T_p\R^2$.\par
	  ...
	  %\[\vx(u,v)=\threevec 001+u\threevec 101+uv\threevec 010\implies \vx\cdot\threevec 10{-1}=-1\]
...
The upshot is that the fundamental forms provide a co-ordinate independent way to compute information about a surface from within the \emph{parametrization space} $U$. We'll think more about this later.
...
\end{example}
\footnotetext{For this reason the first fundamental form is also commonly denoted $\ds^2$.}
...
% We don't even need an explicit surface to compute distance. All we need is a first fundamental form.
% 
% \begin{example}{}{}
% Equip the upper half-plane $\{(u,v):v>0\}$ with the abstract first fundamental form $\I=\frac 1{v^2}\bigl(\du^2+\dv^2\bigr)$. We compute the arc-length of the path $c(t)=\bigl(\cos t,\sin t\bigr)$ between $t=a$ and $b$.\smallbreak
% We have $c'(t)=-\sin t\partials{u}+\cos t\partials{v}$ so that $\I(c',c')=\frac{1}{\sin^2t}$. The arc-length is therefore
% 	\[\int_a^b\frac{1}{\sin t}\dt=\left[-\ln\frac{1+\cos t}{\sin t}\right]_a^b=\ln\frac{\sin b(1+\cos a)}{\sin a(1+\cos b)}\]
% 	Notice that as $a\to 0^+$ or as $b\to \pi^-$ (use l'Hôpital's rule!) the arc-length becomes infinite.\footnotemark
% \end{example}
...
\begin{enumerate}\setcounter{enumi}{1}  
  ...
  \begin{enumerate}
    ...
    %\item Hyperboloid of two sheets $\vx(u,v)=(a\sinh u\cos v,b\sinh u\sin v,c\cosh u)^T$
  \end{enumerate}
  ...
  \begin{enumerate}
    ...
    \item If $\vy(u)=\bigl(\cos \frac u{\sqrt 2},\sin\frac u{\sqrt 2},\frac u{\sqrt 2}\bigr)$ is the unit speed helix, show that%\vspace{-4pt}
  ...
  \item\label{exs:poincarehalfplane} Equip the upper half plane $y>0$ with the abstract first fundamental form $\I=\frac 1{y^2}\bigl(\dx^2+\dy^2\bigr)$. Compute the arc-length between the points $(1,1)$ and $(-1,1)$ in two ways:%\vspace{-4pt}
  \begin{enumerate}
...
\begin{enumerate}\setcounter{enumi}{1}
  ...
  \[\I=(1+4r^2)\,\dr^2+r^2\,\dth^2,\qquad\II=\frac{2}{\sqrt{1+4r^2}}\big(\dr^2+r^2\,\dth^2\bigr)\]
%   \item (Exercise \ref*{sec:fund}.\ref{exs:revfund}) \ When parametrized in the usual manner, the standard co-ordinates of a surface of revolution $\vx(\theta,v)$ are curvature line.
\end{enumerate}
\end{examples}
...
\footnotetext{For all non-zero vectors, $\vec v^TA\vec v>0$. Equivalently, all eigenvalues of $A$ are positive. This means that $\ip{\vec v,\vec w}:=\vec v^TA\vec w$ defines an \emph{inner product} on $\R^n$.}
...
\footnotetext{Since $A$ is symmetric, it has an orthogonal eigenbasis $\{\vec x_1,\ldots,\vec x_n\}$. Since each eigenvalue $\mu_i$ is positive, we may scale the eigenvectors such that $\Nm{\vec x_i}^2=\frac 1{\mu_i}$. Let $X=(\vec x_1\ \cdots\ \vec x_n)$ so that $X^TAX=I$ is the identity matrix. But then,
\begin{gather*}
\det(B-\lambda A) =\det(X^T)^{-1}\det\bigl(X^TBX-\lambda I\bigr)\det(X^{-1}) =0\iff \det(X^TBX-\lambda I)=0
\end{gather*}
Since $X^TBX$ is symmetric, it also has an orthogonal eigenbasis $\{\vec w_1,\ldots,\vec w_n\}$ and real eigenvalues $\lambda_k$. Each $\vec v_k:=X\vec w_k$ is then an eigenvector of $B$ with respect to $A$ with eigenvalue $\lambda_k$; since $X$ is invertible, $\{\vec v_1,\ldots,\vec v_n\}$ is a basis.}


%\skip\footins=-\bigskipamount
\boldsubsubsection{Application to Surfaces}
...
%\[\vec s=s_1\partials u+s_2\partials v,\quad \vec t=t_1\partials u+\vec t_2\partials v\]
...
F&G\end{pmatrix}\right) = (EG-F^2)\lambda^2-(lG+nE-2mF)\lambda+(ln-m^2)\]
	of which $K$ and $H$ are the product and half the sum of the roots. %Note that $EG-F^2=\det A>0$ since $A$ is positive-definite.
...
\footnotetext{If $\vx(p)$ is umbilic, then the eigenspace at $p$ is 2-dimensional and $\lim\limits_{q\to p}\vec v(q)$ might not exist.}
...
\boldsubsubsection{Curvature-Line Co-ordinates}
...
In general this is very unlikely to work, though co-ordinate functions can always be approximated numerically.%\smallbreak
% 
% We may also ask the question of precisely when vector fields $\vec s,\vec t$ are co-ordinate fields. A necessary condition is immediate from the equality of mixed partial derivatives: if $\vec s=\partials s,\vec t=\partials t$ are co-ordinate vector fields, then the \emph{Lie bracket} vanishes,
% 	\[[\vec s,\vec t]=\left[\partials s,\partials t\right]=\partials s\partials t-\partials t\partials s=0\]
% 	In fact this is a locally \emph{sufficient} condition for $\vec s,\vec t$ to be co-ordinate fields: here is a sketch argument.
% 	\begin{itemize}
% 	  \item Given $\vec s,\vec t$ we may define their \emph{dual 1-forms}
% 	  \[\alpha(\vec s)=1=\beta(\vec t)\quad \text{and}\quad \alpha(\vec t)=0=\beta(\vec s)\]
% 	  \item If $[\vec s,\vec t]=0$, then the formula in Exercise \ref{exs:formslie} says that
% 	  \[\D\alpha(\vec s,\vec t)=\vec s[\alpha(\vec t)]-\vec t[\alpha(\vec t)]-\alpha\bigl([\vec s,\vec t])=0\]
% 	  \item Fix a point $p\in U$ and define co-ordinate functions locally by $s(q):=\int_p^q \alpha$ and $t(q)=\int_p^q\beta$ where we integrate along any path. The fact that $\D\alpha=\D\beta=0$ is enough to guarantee that the integrals are path-independent. Indeed $\alpha=\ds$ and $\beta=\dt$.
% 	\end{itemize}
...
% We offer a contrived example where curvature-line directions may be found explicitly.\footnote{Contrived because it arises from a reparametrized cylinder $\vx(u,v)=\frac 12\bigl(\cos\frac{u^2+v^2}{\sqrt 2},\sin\frac{u^2+v^2}{\sqrt 2},\frac{u^2-v^2}{\sqrt 2}\bigr)$, which is regular if and only if $uv\neq 0$.}
% 
% \begin{example}{}{frobenius}
% Suppose a surface has fundamental forms
% \[\I=u^2\,\du^2+v^2\,\dv^2,\qquad \II=u^2\,\du^2+2uv\,\du\dv+v^2\,\dv^2\]
% The principal curvatures are the solutions $k$ to the equation
% 	\[0=\det
% 	\begin{pmatrix}
% 	u^2-k u^2&uv\\uv&v^2-k v^2
% 	\end{pmatrix} =u^2v^2\left[(1-k)^2-1\right]\implies k_1=0,\ k_2=2\]
% 	The principal curvatures are both constant! The curvature directions are easily found:
% 	\begin{align*}
% 	  &k_1=0\!:\ \begin{pmatrix}
% 	u^2&uv\\uv&v^2
% 	  \end{pmatrix}\twovec{s_1}{s_2}=\twovec 00 &&\implies \text{choose }\ \vec s= v\partials u-u\partials v\hspace{130pt}\\
% 	  &k_2=2\!:\ \begin{pmatrix}
% 	-u^2&uv\\uv&-v^2
% 	\end{pmatrix}\twovec{t_1}{t_2}=\twovec 00 &&\implies \text{choose }\ \vec t=v\partials u+u\partials v
% 	\end{align*}
% For co-ordinates, symmetry suggests we try $t=u^2+v^2$: indeed
% \[\vec s[t]=\left(v\partials u-u\partials v\right)(u^2+v^2)=2uv-2uv=0\]
% Similarly, if we let $s=u^2-v^2$, we see that
% \[\vec t[s]=\left(v\partials u+u\partials v\right)(u^2-v^2)=2uv-2uv=0\]
% We conclude that $s=u^2-v^2$ and $t=u^2+v^2$ are curvature-line co-ordinates. In case you're having trouble believing this, compute the differentials:
%   \[\begin{cases}
%   \ds=2u\,\du-2v\,\dv\\
%   \dt=2u\,\du+2v\,\dv
%   \end{cases}\implies
...
%   \II=\frac 14\,\dt^2
%   \end{cases}\]
%   Both fundamental forms are diagonal! We can also compute the co-ordinate vector fields:
%   \[\partials s=\frac 1u\partials u-\frac 1v\partials v=\frac 1{uv}\vec s\quad\text{and}\quad \partials t=\frac 1u\partials u+\frac 1v\partials v=\frac 1{uv}\vec t\]
%   These are parallel to but do not equal $\vec s,\vec t$. With reference to ($\ast$), we have $f=g=\frac 1{uv}$.
% \end{example}
% 
...
% 
% 
% \begin{thm}{Frobenius: hard version}{}
% \begin{enumerate}
%   \item Suppose $\vec s,\vec t$ are linearly independent vector fields on an open set $U$ containing $p$.
% \begin{enumerate}
%   \item If the Lie bracket $[\vec s,\vec t]=\vec s\vec t-\vec t\vec s$ of the vector fields is zero, then there exists a neighborhood $V\subseteq U$ of $p$ and functions $s,t:V\to\R$ such that
%   \[\vec s=\partials s,\qquad \vec t=\partials t\]
%   \item There exists a neighborhood $W\subseteq U$ of $p$ and functions $f,g:W\to\R$ such that $[fX,gY]=0$ on $W$. Part (a) then says there exists $V\subseteq W$ and co-ordinates $s,t:W\to\R$ such that $\partials s=fX$ and $\partials t=gY$ are parallel to $X,Y$.
% \end{enumerate}
% \item If $\vx(p)$ is a non-umbilic point of a surface $\vx:U\to\E^3$, then there exist curvature-line co-ordinates for $\vx$ on some neighborhood of $p$. Indeed if $X,Y$ are curvature directions such that $[X,Y]=0$, let $\alpha,\beta$ be the dual 1-forms to $X,Y$, then (locally) $\alpha=\ds$ and $\beta=\dt$.
% \end{enumerate} 
% \end{thm}
% 
% The proof is a little too technical for this course: at its heart is the usual existence and uniqueness theorem for differential equations and a bit more technology on forms than we've yet developed. It is moreover completely generalizable to arbitrary dimensions.
% 
% \begin{example*}{\ref{ex:frobenius}, mk.\,III}{}
% Frobenius' Theorem actually tells us how to find curvature co-ordinates. We give a sketch, though in practice this involves invoking the existence of solutions to certain differential equations!
% \begin{enumerate}
%   \item Hunt for functions $f,g$ such that the vector fields $f\vec s,g\vec t$ satisfy the Lie bracket condition $[f\vec s,g\vec t]=0$. If we multiply this out (it's messy), this is equivalent to solving the PDE system
%  	\[\begin{cases}
%  	v\partials[f]{u}+u\partials[f]{v}=-f\frac{u^2+v^2}{uv}\\
%  	v\partials[g]{u}-u\partials[g]{v}=-g\frac{u^2-v^2}{uv}
%  	\end{cases}\tag{$\ast$}\]
%  	That this system can be solved is essentially the theorem. Indeed you can check that $f=g=\frac 1{uv}$ does the job!
% 	\item The co-ordinate direction vector fields
%  	\[\widetilde{X}_1=\frac 1u\partials u-\frac 1v\partials v\quad\text{and}\quad
%  	\widetilde{X}_2=\frac 1u\partials u+\frac 1v\partials v\]
%  	satisfy the Lie bracket condition. The dual basis 1-forms to these fields are easy to find using linear algebra:
%  	\[\alpha=\frac 12(u\,\du-v\,\dv)\quad\text{and}\quad \beta=\frac 12(u\,\du+v\,\dv)\]
%  	\item Observe that $\alpha=\ds$ and $\beta=\dt$ where $s=u^2-v^2$ and $t=u^2+v^2$. We therefore have curvature line co-ordinates $s$ and $t$.
% \end{enumerate}
% There are sometimes short-cuts to this procedure if you are feeling lucky, but in the abstract this is what is needed. For instance, we could have gone straight to $\alpha,\beta$ by noting that we have to have $\alpha(X_2)=0=\beta(X_1)$: by luck we would observe that $\alpha,\beta$ are the exterior derivatives of some functions and we're done! In practice, all this trick does is transform the problem to an equivalent system of PDE. In general, given independent vector fields $X_1,X_2$ on $U\subseteq\R^2$, we know that there exist functions $a,b$ such that
% \[[X_1,X_2]=aX_1+bX_2\]
% Hunting for functions $f,g$ such that $[fX_1,gX_2]=0$ is equivalent to $f,g$ satisfying the PDEs
% \[X_2[f]=af\qquad X_1[g]=-bg\]
% This is the system $(\ast)$ in our example, and the problem of finding curvature-line co-ordinates essentially boils down to solving this. The theorem assures us that solutions \emph{exist}, so we can always \emph{assume} the existence of local curvature co-ordinates. Finding them explicitly may be essentially impossible!
% 
% \end{example*}


% \begin{aside}
% {\bf Complete surfaces and umbilic points}\ \ A complete surface is a bounded surface which has no edges (neither open nor closed). A sphere or an ellipsoid are examples. A closed piece of a sphere is no good either since it has an edge. A complete surface has \emph{genus $g$} if it has $g$ holes: thus the sphere has genus 0 and the torus has genus 1.
% 
% \begin{thm}
...
% 
% 	\item Check the computation of the fundamental forms in Example \ref{ex:frobenius} for the surface $\vx(u,v)=\frac 12\bigl(\cos\frac{u^2+v^2}{\sqrt 2},\sin\frac{u^2+v^2}{\sqrt 2},\frac{u^2-v^2}{\sqrt 2}\bigr)$.
...
defines a M\"obius strip in $\E^3$. Show directly that $\vx$ is non-orientable, and that its Gaussian curvature is
