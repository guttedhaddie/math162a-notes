...
\begin{example*}{\ref*{ex:polar}.\ref*{ex:polar2} cont}{}
...
\footnotetext{\label{fn:poincare} A $k$-form $\alpha$ is \emph{closed} if $\D\alpha=0$, and \emph{exact} if $\exists\beta$ such that $\alpha=\D\beta$. The result says that every exact form is closed. \emph{Poincare's Lemma} gives a partial converse: every closed form on an open ball/hypercube is exact. Exercise \ref*{sec:1form}.\ref{exs:exactclosed2} is a simple version.}
